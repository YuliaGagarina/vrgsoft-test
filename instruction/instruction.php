...
поэтому написала то, что написала. 
Попытки его реализовать в файле resources/js/app.js (закомментированные строки).
А также в обоих контроллерах в методах store и update через проверку на запросы 
я написала обработку данных. Проверить не получилось - 500 ошибка.
!!! Очень, вот прям очень прошу развернутый комментарий и по поводу отправки данных,
и по поводу контроллеров !!! Мне очень важно понимать, в верном ли направлении я 
вообще, в принципе, мыслю. Потому что мне кажется, что я написала какую-то дичь)
4. В роутах у меня не очень получилось сделать обновление через PATCH метод.
Там помечено. И за пояснения об этой ошибке тоже буду очень благодарна.
